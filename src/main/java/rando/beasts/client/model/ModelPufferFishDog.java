...
public class ModelPufferFishDog extends EntityModel<EntityPufferfishDog> {
	...
	public void render(EntityPufferfishDog entity, float f, float f1, float f2, float f3, float f4, float f5) {
		...
		if (entity instanceof EntityPufferfishDog && entity.getInflated())
			this.body.render(f5);
		else
	...
	@Override
	public void setRotationAngles(EntityPufferfishDog entityIn, float limbSwing, float limbSwingAmount,
		...
		if (this.state == State.PARTY) {
			...
			this.body_1.rotateAngleX = (float) Math.signum(entityIn.getMotion().x) / 16;
			this.body_1.rotateAngleZ = (float) Math.signum(entityIn.getMotion().z) / 16;
			this.leftArm_1.rotateAngleX = MathHelper.cos(limbSwing * degree) * speed * limbSwingAmount;
			this.rightArm_1.rotateAngleX = MathHelper.cos(limbSwing * degree + (float) Math.PI) * speed
					* limbSwingAmount;
			this.leftLeg_1.rotateAngleX = MathHelper.cos(limbSwing * degree + (float) Math.PI) * speed
					* limbSwingAmount;
	...
	@Override
	public void setLivingAnimations(EntityPufferfishDog entitylivingbaseIn, float limbSwing, float limbSwingAmount,
		...
		} else if (entitylivingbaseIn.getMotion().length() != 0) {
			resetRotationPoints();
	...
	@OnlyIn(Dist.CLIENT)
	static enum State {
		WALKING, STANDING, SITTING, PARTY;
	}
}
