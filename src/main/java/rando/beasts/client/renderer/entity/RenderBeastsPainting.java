...
public class RenderBeastsPainting extends EntityRenderer<EntityBeastsPainting> {
	...
	@Override
	public void doRender(EntityBeastsPainting entity, double x, double y, double z, float entityYaw,
			float partialTicks) {
	...
	protected ResourceLocation getEntityTexture(EntityBeastsPainting entity) {
		return TEXTURES.putIfAbsent(entity.art, new ResourceLocation(BeastsReference.ID,
				"textures/painting/" + entity.art.title.toLowerCase().replace(" ", "_") + ".png"));
	}
	...
	private void setLightmap(EntityBeastsPainting BeastsPainting, float p_77008_2_, float p_77008_3_) {
		...
		if (enumfacing == Direction.NORTH)
			i = MathHelper.floor(BeastsPainting.posX + (p_77008_2_ / 16.0F));
		if (enumfacing == Direction.WEST)
			k = MathHelper.floor(BeastsPainting.posZ - (p_77008_2_ / 16.0F));
		...
		int i1 = l % 65536;
		int j1 = l / 65536;
		GLX.glMultiTexCoord2f(GLX.GL_TEXTURE1, i1, j1);
